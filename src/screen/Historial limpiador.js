...
import HistorialCardLimpiador from '../components/HistorialCardLimpiador';

export default function HistorialLimpiador() {
		...
		React.useCallback(() => {
		  const loadProfile = async () => {
			try {
			  ...
			  if (token) {
				const url = baseUrl + '/api/servicios/byidlimpiador/' + id;
				try {
		...
		<SafeAreaView>
		  ...
		  {servicios === 'No posee un historial aún.' ? (
			...
			<FlatList
			  ...
			  renderItem={({ item, index }) => <HistorialCardLimpiador servicios={item} />}
			/>
		  )}
		</SafeAreaView>
	  );
